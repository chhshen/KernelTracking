/**
 * modified version based on the f2c version    
 */
...
static int mcsrch_();
static int mcsrch_(integer *n, doublereal *x, doublereal *f, doublereal *g, doublereal *s, doublereal *stp, doublereal *ftol, doublereal *xtol, 
									integer *maxfev, integer *info, integer *nfev, doublereal *wa);
static int mcstep_();
static int mcstep_(doublereal *stx, doublereal *fx, doublereal *dx, doublereal *sty, doublereal *fy, 
...
/* Subroutine */ int lbfgs(integer *n, integer *m, doublereal *x, doublereal *f, doublereal *g, 
						   integer *diagco, doublereal *diag, integer *iprint, 
...
/* Subroutine */ static int mcsrch_(integer *n, doublereal *x, doublereal *f, doublereal *g, 
									doublereal *s, doublereal *stp, doublereal *ftol, doublereal *xtol, 
									integer *maxfev, integer *info, integer *nfev, doublereal *wa)
{
    /* Initialized data */
...
L30:
    ...
    *stp = MAX_(*stp,lb3_1.stpmin);
    *stp = MIN_(*stp,lb3_1.stpmax);
...
L45:
    *info = 0;
    ...
    if (*f <= ftest1 && ABS_(dg) <= lb3_1.gtol * (-dginit)) {
        *info = 1;
    }
    ...
    if (stage1 && *f <= ftest1 && dg >= MIN_(*ftol,lb3_1.gtol) * dginit) {
        stage1 = FALSE_;
    }
    ...
    if (brackt) {
        if ((d__1 = sty - stx, ABS_(d__1)) >= p66 * width1) {
            *stp = stx + p5 * (sty - stx);
        }
        width1 = width;
...
/* Subroutine */ static int mcstep_(doublereal *stx, doublereal *fx, doublereal *dx, doublereal *sty, doublereal *fy, 
									doublereal *dy, doublereal *stp, doublereal *fp, doublereal *dp, 
									logical *brackt, 
                                    doublereal *stpmin, doublereal *stpmax, integer *info)
{
...
//    double sqrt();
    ...
    *info = 0;
    ...
    if (*brackt && ((*stp <= MIN_(*stx,*sty) || *stp >= MAX_(*stx,*sty)) || *dx *
    ...
    if (*fp > *fx) {
        ...
        *info = 3;
        ...
        stpq = *stp + *dp / (*dp - *dx) * (*stx - *stp);
        if (*brackt) {
            if ((d__1 = *stp - stpc, ABS_(d__1)) < (d__2 = *stp - stpq, ABS_(
                                                                             d__2))) {
                stpf = stpc;
            } else {
                stpf = stpq;
